...
class HabitValidators:
    ...
    def validate_reward_and_related(habit):
        ...
        if habit.reward and habit.related_habit:
            raise ValidationError(
                {'non_field_errors': _('Нельзя одновременно указывать вознаграждение и связанную привычку')}
            )
    ...
    def validate_execution_time(habit):
        ...
        if habit.execution_time > 120:
            raise ValidationError({'execution_time': _('Время выполнения не должно превышать 120 секунд')})
    ...
    def validate_related_habit(habit):
        ...
        if habit.related_habit and not habit.related_habit.is_pleasant:
            raise ValidationError({'related_habit': _('Связанная привычка должна быть приятной')})
    ...
    def validate_pleasant_habit(habit):
        ...
        if habit.is_pleasant:
            if habit.reward and habit.related_habit:
                raise ValidationError(
                    {'non_field_errors': _('Приятная привычка не может иметь вознаграждение или связанную привычку')}
                )
            elif habit.reward:
                raise ValidationError({'reward': _('Приятная привычка не может иметь вознаграждение')})
            elif habit.related_habit:
                raise ValidationError({'related_habit': _('Приятная привычка не может иметь связанную привычку')})

    @staticmethod
    def validate_periodicity(habit):
        """Периодичность не должна быть больше 7 дней"""
        if habit.periodicity > 7:
            raise ValidationError({'periodicity': _('Нельзя выполнять привычку реже, чем 1 раз в 7 дней')})
